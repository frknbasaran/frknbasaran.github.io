        ...
        **/
        this.present_feedback_widget = function(presentableFeedback) {
            ...
            if (presentableFeedback.type === "rating") {
                processWidget(presentableFeedback._id);
            }
            else {
                try {
                    var url = this.url;
                    ...
                    if (presentableFeedback.type === "survey") {
                        ...
                        console.log(presentableFeedback);
                    }
                }
                catch (e) {
        ...
        function processWidget(widgetID) {
            sendXmlHttpRequest(Countly.url + '/o/feedback/widget', {widget_id: widgetID}, function(err, params, responseText) {
                if (err) {
                    log("Error occurred", err);
                }
                try {
                    // inject feedback styles
                    loadCSS(Countly.url + '/star-rating/stylesheets/ratings-sticker.css');
                    // inject popup background layer
                    document.body.insertAdjacentHTML('beforeend', '<div id="cfbg"></div>');
                    ...
                    try {
                        ...
                        if (!currentWidget.hide_sticker) {
                            ...
                            add_event(document.getElementById('countly-feedback-sticker-' + currentWidget._id), 'click', function() {
                                ...
                                sendXmlHttpRequest(self.url + '/i/feedback/show-popup', { widget_id: currentWidget._id, app_key: self.app_key }, function() {});
                            });
                        }
                        else {
                            document.getElementById('countly-iframe-wrapper-' + currentWidget._id).style.display = "block";
