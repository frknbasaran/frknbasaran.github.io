$(function() {
	...
	*/
	$('body').append('<div class="modal"> <div class="modal-content"> <div class="emotions-area"> <p id="question-area">What is your opinion of this page?</p><span class="emotion"><img class="rating-emotion" data-score="1" src="img/0_gray.svg"></span><span class="emotion"><img class="rating-emotion" data-score="2" src="img/1_gray.svg"></span><span class="emotion"><img class="rating-emotion" data-score="3" src="img/2_gray.svg"></span><span class="emotion"><img class="rating-emotion" data-score="4" src="img/3_gray.svg"></span><span class="emotion"><img class="rating-emotion" data-score="5" src="img/4_gray.svg"></span> </div><div class="comment-area"> <div class="input-wrapper"> <input id="comment-show" type="checkbox"> <label for="comment-show">Add Comment</label> <textarea id="countly-feedback-comment-textarea"></textarea> </div><div class="input-wrapper"> <input id="email-show" type="checkbox"> <label for="contact-me-email">Contact me by e-mail</label> <input type="text" id="contact-me-email"> </div></div><div class="buttons-area"><button id="close-button">Cancel</button><button id="send-button">Submit Feedback</button> </div><div class="modal-footer"> <img src="img/powered-by-countly.svg" id="powered-by-countly"> </div></div></div><div id="countly-feedback">Feedback</div><div class="success-modal"> <div class="success-modal-content"> <div class="icon-area"> <i class="fa fa-check fa-2x"></i> </div><div class="emotions-area"> <p id="question-area">Thank you.<br>We received your message.</p></div><div class="buttons-area-on-success"> <button id="continue-button">Continue</button> </div><div class="modal-footer"> <img src="img/powered-by-countly.svg" id="powered-by-countly"> </div></div></div>');
	...
	function rate(index) {
		event["segmentation"].rating = parseInt(modalEmotionImages[index].getAttribute('data-score'));
	}
	...
	function sendFeedback() {
		event["segmentation"].comment = document.getElementById('countly-feedback-comment-textarea').value;
		event["segmentation"].email = document.getElementById('contact-me-email').value;
		try {
			...
			x.send({"method":"star","events":JSON.stringify([event]), "app_key":constants.app_key, "device_id":1,"timestamp":timestamp()});
		} catch (e) {
	...
	for (var i = 0; i < modalEmotionImages.length; i++) {
		if(modalEmotionImages[0].addEventListener) 
			modalEmotionImages[0].addEventListener('click', rate, false);
		// this is for IE, because it doesn't support addEventListener
		else if(modalEmotionImages[0].attachEvent) 
			// this strange part for making the keyword 'this' indicate the clicked anchor
			modalEmotionImages[0].attachEvent('onclick', function() { return rate.apply(i, modalCloseButton, [window.event])}); 	
	}
	...
	$('.emotion > img').on('click', function() {
		...
		$(this).attr('src','img/'+(parseInt($(this).data('score'))-1) + '_color.svg');
	})
